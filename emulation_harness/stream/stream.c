...
double STREAM_Read64_Random(uint64_t *read_checksum) {
	...
	for (j=0; j<READ64_ARRAY_COUNT; j++) {
		VEC_TYPE mm_a = VEC_LOAD(&a[8*(x & (READ64_ARRAY_COUNT - 1))]);
...
double STREAM_Read64_Random_Chunk2(uint64_t *read_checksum) {
	...
	for (j=0; j<READ64_ARRAY_COUNT_CHUNK2; j++) {
		for(k = 0; k < 2; k++) {
			VEC_TYPE mm_a = VEC_LOAD(&a[2*8*(x & (READ64_ARRAY_COUNT_CHUNK2 - 1)) + 8*k]);
			sum = VEC_ADD(sum, mm_a);
		}
		x ^= x << 13;
		x ^= x >> 7;
...
double STREAM_Read64_Random_Chunk4(uint64_t *read_checksum) {
	...
	for (j=0; j<READ64_ARRAY_COUNT_CHUNK4; j++) {
		for(k = 0; k < 4; k++) {
			VEC_TYPE mm_a = VEC_LOAD(&a[4*8*(x & (READ64_ARRAY_COUNT_CHUNK4 - 1)) + 8*k]);
			sum = VEC_ADD(sum, mm_a);
		}
		x ^= x << 13;
		x ^= x >> 7;
...
double STREAM_Read64_Random_Chunk8(uint64_t *read_checksum) {
	...
	for (j=0; j<READ64_ARRAY_COUNT_CHUNK8; j++) {
		for(k = 0; k < 8; k++) {
			VEC_TYPE mm_a = VEC_LOAD(&a[8*8*(x & (READ64_ARRAY_COUNT_CHUNK8 - 1)) + 8*k]);
			sum = VEC_ADD(sum, mm_a);
		}
		x ^= x << 13;
		x ^= x >> 7;
...
double STREAM_Read64_Random_Chunk16(uint64_t *read_checksum) {
	...
	for (j=0; j<READ64_ARRAY_COUNT_CHUNK16; j++) {
		for(k = 0; k < 16; k++) {
			VEC_TYPE mm_a = VEC_LOAD(&a[16*8*(x & (READ64_ARRAY_COUNT_CHUNK16 - 1)) + 8*k]);
			sum = VEC_ADD(sum, mm_a);
		}
		x ^= x << 13;
		x ^= x >> 7;
...
double STREAM_Read64_Random_Chunk32(uint64_t *read_checksum) {
	...
	for (j=0; j<READ64_ARRAY_COUNT_CHUNK32; j++) {
		for(k = 0; k < 32; k++) {
			VEC_TYPE mm_a = VEC_LOAD(&a[32*8*(x & (READ64_ARRAY_COUNT_CHUNK32 - 1)) + 8*k]);
			sum = VEC_ADD(sum, mm_a);
		}
		x ^= x << 13;
		x ^= x >> 7;
...
double STREAM_Read64_Random_Chunk64(uint64_t *read_checksum) {
	...
	for (j=0; j<READ64_ARRAY_COUNT_CHUNK64; j++) {
		for(k = 0; k < 64; k++) {
			VEC_TYPE mm_a = VEC_LOAD(&a[64*8*(x & (READ64_ARRAY_COUNT_CHUNK64 - 1)) + 8*k]);
			sum = VEC_ADD(sum, mm_a);
		}
		x ^= x << 13;
		x ^= x >> 7;
...
double STREAM_Read64_Random_Chunk128(uint64_t *read_checksum) {
	...
	for (j=0; j<READ64_ARRAY_COUNT_CHUNK128; j++) {
		for(k = 0; k < 128; k++) {
			VEC_TYPE mm_a = VEC_LOAD(&a[128*8*(x & (READ64_ARRAY_COUNT_CHUNK128 - 1)) + 8*k]);
			sum = VEC_ADD(sum, mm_a);
		}
		x ^= x << 13;
		x ^= x >> 7;
...
double STREAM_ReadWrite64(uint64_t *read_checksum) {
	...
	for (j=0; j<STREAM_ARRAY_SIZE; j += 8) {
		VEC_TYPE mm_a = VEC_LOAD(&a[j]);
		VEC_STORE(&a[j], VEC_ADD(mm_a, val));
	}
...
double STREAM_ReadWrite64_Random(uint64_t *read_checksum) {
	...
	for (j=0; j<STREAM_ARRAY_SIZE; j += 8) {
		VEC_TYPE mm_a = VEC_LOAD(&a[8*(x & (READ64_ARRAY_COUNT - 1))]);
		VEC_STORE(&a[8*(x & (READ64_ARRAY_COUNT - 1))], VEC_ADD(mm_a, val));
		x ^= x << 13;
		x ^= x >> 7;
		x ^= x << 17;
	}
...
// Available workloads (add this as a comment/documentation):
/*
 * AVAILABLE STREAM WORKLOADS:
 * 
 * Sequential Access Patterns:
 * - "Read16"          - Sequential 128-bit (16-byte) reads using SSE
 * - "Write16"         - Sequential 128-bit writes using SSE  
 * - "ReadWrite16"     - Sequential 128-bit read-modify-write using SSE
 * - "Read64"          - Sequential 512-bit (64-byte) reads using AVX-512
 * - "Write64"         - Sequential 512-bit writes using AVX-512
 * - "ReadWrite64"     - Sequential 512-bit read-modify-write using AVX-512
 * - "NtWrite64"       - Sequential 512-bit non-temporal writes (bypass cache)
 * - "Triad"           - Traditional STREAM triad: a[j] = b[j] + 3.0*c[j]
 *
 * Random Access Patterns:
 * - "Read64Random"         - Random 512-bit reads
 * - "Read64RandomChunk2"   - Random reads of 2x 512-bit chunks
 * - "Read64RandomChunk4"   - Random reads of 4x 512-bit chunks  
 * - "Read64RandomChunk8"   - Random reads of 8x 512-bit chunks
 * - "Read64RandomChunk16"  - Random reads of 16x 512-bit chunks
 * - "Read64RandomChunk32"  - Random reads of 32x 512-bit chunks
 * - "Read64RandomChunk64"  - Random reads of 64x 512-bit chunks
 * - "Read64RandomChunk128" - Random reads of 128x 512-bit chunks
 * - "NtWrite64Random"      - Random 512-bit non-temporal writes
 * - "ReadWrite64Random"    - Random 512-bit read-modify-write
 *
 * Usage: ./stream <workload> <duration-in-seconds>
 * Example: ./stream Read64 10
 */
