...
if [ ! -f $PARTITION_PATH ]; then
    ...
    reset_hugepage_allocation
    GALOIS_DO_NOT_BIND_THREADS=1 time mpirun -n $NUM_PROCS --map-by socket:PE=5 --use-hwthread-cpus --report-bindings $GROUDON_ADDAGG -g $GRAPH_PATH -c 1 -m $NUM_PARTITIONS -t 20 -a pr  2>&1 | tee $LOG_DIR/pr_$LOG_SUFFIX;
    reset_hugepage_allocation
    GALOIS_DO_NOT_BIND_THREADS=1 time mpirun -n $NUM_PROCS --map-by socket:PE=5 --use-hwthread-cpus --report-bindings $GROUDON_MINAGG -g $SYM_GRAPH_PATH -c 1 -m $NUM_PARTITIONS -t 20 -a cc  2>&1 | tee $LOG_DIR/cc_$LOG_SUFFIX;
    reset_hugepage_allocation
    GALOIS_DO_NOT_BIND_THREADS=1 time mpirun -n $NUM_PROCS --map-by socket:PE=5 --use-hwthread-cpus --report-bindings $GROUDON_MINAGG -g $GRAPH_PATH -c 1 -m $NUM_PARTITIONS -t 20 -a sssp  2>&1 | tee $LOG_DIR/sssp_$LOG_SUFFIX;
else
    LOG_SUFFIX=Groudon_"$GRAPH_NAME"_"$NUM_PARTITIONS"N.log
    reset_hugepage_allocation
    GALOIS_DO_NOT_BIND_THREADS=1 time mpirun -n $NUM_PROCS --map-by socket:PE=5 --use-hwthread-cpus --report-bindings $GROUDON_ADDAGG -g $GRAPH_PATH -c 1 -m $NUM_PARTITIONS -t 20 -a pr -p $PARTITION_PATH   2>&1 | tee $LOG_DIR/pr_$LOG_SUFFIX;
    reset_hugepage_allocation
    GALOIS_DO_NOT_BIND_THREADS=1 time mpirun -n $NUM_PROCS --map-by socket:PE=5 --use-hwthread-cpus --report-bindings $GROUDON_MINAGG -g $SYM_GRAPH_PATH -c 1 -m $NUM_PARTITIONS -t 20 -a cc -p $PARTITION_PATH  2>&1 | tee $LOG_DIR/cc_$LOG_SUFFIX;
    reset_hugepage_allocation
    GALOIS_DO_NOT_BIND_THREADS=1 time mpirun -n $NUM_PROCS --map-by socket:PE=5 --use-hwthread-cpus --report-bindings $GROUDON_MINAGG -g $GRAPH_PATH -c 1 -m $NUM_PARTITIONS -t 20 -a sssp -p $PARTITION_PATH  2>&1 | tee $LOG_DIR/sssp_$LOG_SUFFIX;
fi