...
LOG_DIR="$GROUDON_HOME/logs/l3_emulation_runs"
...
do
    ...
    GALOIS_DO_NOT_BIND_THREADS=1 time mpirun -n 3 --rankfile $RUN_SCRIPTS_DIR/gluon_proc_map --use-hwthread-cpus --report-bindings $GALOIS_HOME/pagerank/pagerank-push-dist $GRAPH_PATH -exec=Sync  -runs=1 -t=$COMPUTE_NODE_CORES 2>&1 | tee $LOG_DIR/pr_$LOG_SUFFIX;
    # reset_hugepage_allocation
...
do 
    ...
    reset_hugepage_allocation
    GALOIS_DO_NOT_BIND_THREADS=1 time mpirun -n 3 --rankfile $RUN_SCRIPTS_DIR/graphq_proc_map --use-hwthread-cpus --report-bindings $GALOIS_HOME/pagerank/pagerank-push-dist $GRAPH_PATH -exec=Sync  -runs=1 -t=$MEMORY_NODE_LCORES 2>&1 | tee $LOG_DIR/pr_$LOG_SUFFIX;
    # reset_hugepage_allocation
...
if [ ! -f $PARTITION_PATH ]; then
    for i in {1..3}; 
    do
        LOG_SUFFIX=FAMGraphOpt_"$GRAPH_NAME"_3N_run$i.log 
        reset_hugepage_allocation
        GALOIS_DO_NOT_BIND_THREADS=1 time mpirun -n 2 --rankfile $RUN_SCRIPTS_DIR/gluon_proc_map --use-hwthread-cpus --report-bindings $GROUDON_BIN -g $GRAPH_PATH -c 1 -m 3 -t $MEMORY_NODE_LCORES -a pr -o 0 2>&1 | tee $LOG_DIR/pr_$LOG_SUFFIX;
        # reset_hugepage_allocation
        # GALOIS_DO_NOT_BIND_THREADS=1 time mpirun -n 4 --rankfile $RUN_SCRIPTS_DIR/groudon_proc_map --use-hwthread-cpus --report-bindings $GROUDON_BIN -g $SYM_GRAPH_PATH -c 1 -m 3 -t $MEMORY_NODE_LCORES -a cc  2>&1 | tee $LOG_DIR/cc_$LOG_SUFFIX;
        # reset_hugepage_allocation
        # GALOIS_DO_NOT_BIND_THREADS=1 time mpirun -n 4 --rankfile $RUN_SCRIPTS_DIR/groudon_proc_map --use-hwthread-cpus --report-bindings $GROUDON_BIN -g $GRAPH_PATH -c 1 -m 3 -t $MEMORY_NODE_LCORES -a sssp 2>&1 | tee $LOG_DIR/sssp_$LOG_SUFFIX;
    ...
    do
        LOG_SUFFIX=FAMGraphOpt_"$GRAPH_NAME"_3N_run$i.log 
        reset_hugepage_allocation
        GALOIS_DO_NOT_BIND_THREADS=1 time mpirun -n 2 --rankfile $RUN_SCRIPTS_DIR/gluon_proc_map --use-hwthread-cpus --report-bindings $GROUDON_BIN -g $GRAPH_PATH -c 1 -m 3 -t $MEMORY_NODE_LCORES -a pr -p $PARTITION_PATH -o 0 2>&1 | tee $LOG_DIR/pr_$LOG_SUFFIX;
        # reset_hugepage_allocation
        # GALOIS_DO_NOT_BIND_THREADS=1 time mpirun -n 4 --rankfile $RUN_SCRIPTS_DIR/groudon_proc_map --use-hwthread-cpus --report-bindings $GROUDON_BIN -g $SYM_GRAPH_PATH -c 1 -m 3 -t $MEMORY_NODE_LCORES -a cc -p $PARTITION_PATH 2>&1 | tee $LOG_DIR/cc_$LOG_SUFFIX;
        # reset_hugepage_allocation
        # GALOIS_DO_NOT_BIND_THREADS=1 time mpirun -n 4 --rankfile $RUN_SCRIPTS_DIR/groudon_proc_map --use-hwthread-cpus --report-bindings $GROUDON_BIN -g $GRAPH_PATH -c 1 -m 3 -t $MEMORY_NODE_LCORES -a sssp -p $PARTITION_PATH 2>&1 | tee $LOG_DIR/sssp_$LOG_SUFFIX;
